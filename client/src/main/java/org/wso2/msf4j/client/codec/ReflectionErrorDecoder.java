...
/**
 * Feign error decoder which dynamically maps REST service error responses to Java exceptions via Java Reflection API
 *
 * @param <T> A Class representing the body of the response sent on the API in case of error
 * @param <S> A base exception class which all the exceptions declared thrown on the client interface will inherit from
 */
...
public abstract class ReflectionErrorDecoder<T, S extends Exception> implements ErrorDecoder {
    ...
    private static void initDetailMessageField() throws NoSuchFieldException {
        ReflectionErrorDecoder.detailMessageField = Throwable.class.getDeclaredField("detailMessage");
        AccessController.doPrivileged(new PrivilegedAction() {
            @Override
            public Object run() {
                ReflectionErrorDecoder.detailMessageField.setAccessible(true);
                return null;
            }
        });
    }
    ...
    private S getExceptionByReflection(T apiResponse)
        ...
        ReflectionErrorDecoder.detailMessageField.set(exceptionToBeThrown, getMessageFromResponse(apiResponse));
        ...
        if (supplier != null) {
            ...
            if (errorCode == null || errorCode.isEmpty()) {
                throw new IllegalStateException(
                        String.format(
                                "The exception '%s' needs to declare an error code to be rethrown. If it's a base " +
                                        "exception, make it abstract.", clazz.getName()));
            }

            ThrownExceptionDetails<S> existingExceptionDetails =
                    exceptionsThrown.put(errorCode, new ThrownExceptionDetails<S>(clazz, supplier));
